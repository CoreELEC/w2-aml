 ...
 ******************************************************************************
 *
 * @file aml_dini.c - Add support for dini platform
 *
...
static void dini_dma_on(struct aml_dini *aml_dini)
{
    int i;
    u32 reread_time;
    volatile void *reg;
    ...
    for (i = 0; i < ARRAY_SIZE(mv_cfg_fpga_dma_ctrl_regs); i++) {
        ...
        reread_time = w2_aml_pci_readl(reg) & CFPGA_DMA_CTRL_REREAD_TIME_MASK;
        w2_aml_pci_writel(CFPGA_DMA_CTRL_CLEAR | reread_time, reg);
        w2_aml_pci_writel(CFPGA_DMA_CTRL_ENABLE | reread_time, reg);
    }
}

/* This also clears running transactions */
static void dini_dma_off(struct aml_dini *aml_dini)
{
    int i;
    u32 reread_time;
    volatile void *reg;
    ...
    for (i = 0; i < ARRAY_SIZE(mv_cfg_fpga_dma_ctrl_regs); i++) {
        ...
        w2_aml_pci_writel(CFPGA_DMA_CTRL_DISABLE | reread_time, reg);
        w2_aml_pci_writel(CFPGA_DMA_CTRL_CLEAR   | reread_time, reg);
    }
}
...
{
    w2_aml_pci_writel(low, aml_dini->pci_bar0_vaddr + CFPGA_BAR4_LOADDR_REG);
    w2_aml_pci_writel(high, aml_dini->pci_bar0_vaddr + CFPGA_BAR4_HIADDR_REG);
    w2_aml_pci_writel(CFPGA_BAR4_LOADDR_MASK_MAX,
 ...
 */
int aml_cfpga_irq_enable(struct aml_hw *aml_hw)
{
    ...
    if ((ret = request_irq(aml_hw->plat->pci_dev->irq, aml_irq_pcie_hdlr, 0,
                           "aml", aml_hw)))
            return ret;

    reg = aml_dini->pci_bar0_vaddr + CFPGA_UINTR_MASK_REG;
    cfpga_uintr_mask = w2_aml_pci_readl(reg);
    w2_aml_pci_writel(cfpga_uintr_mask | CFPGA_PCIEX_IT, reg);
 ...
 */
int aml_cfpga_irq_disable(struct aml_hw *aml_hw)
{
    struct aml_plat *aml_plat = aml_hw->plat;
    struct aml_dini *aml_dini = (struct aml_dini *)aml_plat->priv;
    unsigned int cfpga_uintr_mask;
...
static int aml_dini_platform_enable(struct aml_hw *aml_hw)
{
    struct aml_plat *aml_plat = aml_hw->plat;
    struct aml_dini *aml_dini = (struct aml_dini *)aml_plat->priv;

#ifdef CONFIG_AML_SDM
    w2_aml_pci_writel(0x0000FFFF, aml_dini->pci_bar0_vaddr + CFPGA_BAR_TOUT);
#endif

    dini_dma_on(aml_dini);
    return aml_cfpga_irq_enable(aml_hw);
}

static int aml_dini_platform_disable(struct aml_hw *aml_hw)
{
    struct aml_plat *aml_plat = aml_hw->plat;
    struct aml_dini *aml_dini = (struct aml_dini *)aml_plat->priv;
    int ret;

    ret = aml_cfpga_irq_disable(aml_hw);
    dini_dma_off(aml_dini);
    return ret;
}

static void aml_dini_platform_deinit(struct aml_plat *aml_plat)
{
    struct aml_dini *aml_dini = (struct aml_dini *)aml_plat->priv;

    pci_disable_device(aml_plat->pci_dev);
    iounmap(aml_dini->pci_bar0_vaddr);
    iounmap(aml_dini->pci_bar4_vaddr);
    pci_release_regions(aml_plat->pci_dev);

    kfree(aml_plat);
}

static u8* aml_dini_get_address(struct aml_plat *aml_plat, int addr_name,
                                 unsigned int offset)
{
    struct aml_dini *aml_dini = (struct aml_dini *)aml_plat->priv;
    ...
    if (addr_name == AML_ADDR_CPU)
        ...
        dini_set_bar4_win(AHB_BRIDGE_WINDOW_LOW, AHB_BRIDGE_WINDOW_HIGH, aml_dini);

    return aml_dini->pci_bar4_vaddr + offset;
}

static u32 aml_dini_ack_irq(struct aml_hw *aml_hw)
{
    struct aml_dini *aml_dini = (struct aml_dini *)aml_hw->plat->priv;
    w2_aml_pci_writel(CFPGA_ALL_ITS, aml_dini->pci_bar0_vaddr + CFPGA_UINTR_SRC_REG);
    return 0;
}

static const u32 aml_dini_config_reg[] = {
...
static int aml_dini_get_config_reg(struct aml_plat *aml_plat, const u32 **list)
{
    if (!list)
        return 0;

    *list = aml_dini_config_reg;
    return ARRAY_SIZE(aml_dini_config_reg);
}

/**
 * aml_dini_platform_init - Initialize the DINI platform
 *
 * @pci_dev PCI device
 ...
 */
int aml_dini_platform_init(struct pci_dev *pci_dev, struct aml_plat **aml_plat)
{
    struct aml_dini *aml_dini = NULL;
    u16 pci_cmd = 0;
    int ret = 0;

    *aml_plat = kzalloc(sizeof(struct aml_plat) + sizeof(struct aml_dini),
                        GFP_KERNEL);
    if (!*aml_plat)
        return -ENOMEM;

    aml_dini = (struct aml_dini *)(*aml_plat)->priv;
    ...
    (*aml_plat)->enable = aml_dini_platform_enable;
    (*aml_plat)->disable = aml_dini_platform_disable;
    (*aml_plat)->deinit = aml_dini_platform_deinit;
    (*aml_plat)->get_address = aml_dini_get_address;
    (*aml_plat)->ack_irq = aml_dini_ack_irq;
