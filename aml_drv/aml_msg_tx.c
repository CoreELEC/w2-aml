...
{
    if ((aml_hw->state !=  WIFI_SUSPEND_STATE_NONE) && (msg->param_len != 0))
    {
        if (msg->id == MM_OTHER_REQ)
        {
            if ((*(msg->param) == MM_SUB_SET_SUSPEND_REQ)
                || (*(msg->param) == MM_SUB_SCANU_CANCEL_REQ)
                || (*(msg->param) == MM_SUB_ARP_AGENT_REQ)
        ...
        else if (msg->id == ME_SET_PS_MODE_REQ)
        {
            return true;
        }
        else
 ...
 ******************************************************************************
 */
static int aml_priv_send_msg(struct aml_hw *aml_hw, const void *msg_params,
                             int reqcfm, lmac_msg_id_t reqid, void *cfm)
{
    void *msg;
...
static int aml_send_bcn_change(struct aml_hw *aml_hw, u8 vif_idx, u32 bcn_addr,
...
static int aml_beacon_update_pre(struct aml_vif *vif, struct aml_ipc_buf *buf, void *bcn, int len)
{
    struct aml_hw *aml_hw = vif->aml_hw;

    if (aml_bus_type == PCIE_MODE) {
        int error = aml_ipc_buf_a2e_init(aml_hw, buf, bcn, len);

        if (error) {
            AML_ERR("%s: failed to allocate IPC buf for new beacon\n", vif->ndev->name);
            return error;
        }
    } else {
        unsigned char *addr = (unsigned char *)(uintptr_t)TXL_BCN_POOL
                              + vif->vif_index * (BCN_TXLBUF_TAG_LEN + NX_BCNFRAME_LEN)
                              + BCN_TXLBUF_TAG_LEN;

        if (aml_bus_type == USB_MODE) {
            aml_hw->plat->hif_ops->hi_write_sram(bcn, addr, len, USB_EP4);
        } else if (aml_bus_type == SDIO_MODE) {
            aml_hw->plat->hif_sdio_ops->hi_random_ram_write(bcn, addr, len);
        }
    }
    return 0;
}
                      ...
                      void *bcn_buf, int addition_ie, u16 *csa_offset)
{
    ...
    int len = bcn->len + addition_ie;
    int error = aml_beacon_update_pre(vif, &buf, bcn_buf, len);

    if (!error) {
        // Forward the information to the LMAC
        error = aml_send_bcn_change(aml_hw, vif->vif_index, buf.dma_addr, len,
                                    bcn->head_len, bcn->tim_len, csa_offset);
...
int aml_send_me_chan_config_req(struct aml_hw *aml_hw)
{
    ...
    struct wiphy *wiphy = aml_hw->wiphy;
    unsigned char ofdm_power = 19;
    unsigned char dsss_power = 21;
    int i;
    ...
    if (wiphy->bands[NL80211_BAND_2GHZ] != NULL) {
        ...
        for (i = 0; i < b->n_channels; i++) {
            ...
            if (regdom_en != 1) {
                ...
                aml_regdom_table_pwr_get(aml_ieee80211_freq_to_chan(b->channels[i].center_freq, NL80211_BAND_2GHZ),
                        NL80211_BAND_2GHZ, &ofdm_power, &dsss_power);
                dsss_power = dsss_power & 0x7f;
                req->chan2G4[req->chan2G4_cnt].tx_power = aml_regdom_set_pwr(chan_to_fw_pwr(b->channels[i].max_power), dsss_power);
            }
            req->chan2G4_cnt++;
    ...
    if (wiphy->bands[NL80211_BAND_5GHZ] != NULL) {
        ...
        for (i = 0; i < b->n_channels; i++) {
            ...
            if (regdom_en != 1) {
                ...
                aml_regdom_table_pwr_get(aml_ieee80211_freq_to_chan(b->channels[i].center_freq, NL80211_BAND_5GHZ),
                        NL80211_BAND_5GHZ, &ofdm_power, &dsss_power);
                ofdm_power = ofdm_power & 0x7f;
                req->chan5G[req->chan5G_cnt].tx_power = aml_regdom_set_pwr(chan_to_fw_pwr(b->channels[i].max_power), ofdm_power);
            }
            req->chan5G_cnt++;
...
int aml_send_me_set_ps_mode(struct aml_hw *aml_hw, u8 ps_mode, bool is_suspend_resume)
{
    ...
    if ((aml_hw->state == WIFI_SUSPEND_STATE_WOW) && (!is_suspend_resume))
    {
        AML_INFO("drv state wow\n");
        return -ENOMEM;
    }
...
static const struct element *aml_get_md_ie(struct aml_vif *vif, struct cfg80211_connect_params *sme)
{
...
int aml_ps_info_get(struct net_device *dev, struct ps_info_s * ind, int debug_type)
{
    ...
    struct ps_info_get_req * ps_info_get_req;

    ps_info_get_req = aml_priv_msg_zalloc(MM_SUB_GET_PS_INFO, sizeof(struct ps_info_get_req));
    if (!ps_info_get_req)
        return -ENOMEM;

    ps_info_get_req->debug_type = debug_type;

    ret = aml_priv_send_msg(aml_hw, ps_info_get_req, 1, PRIV_GET_PS_INFO_CFM, ind);
...
#ifdef CONFIG_AML_RECOVERY
    if ((!ret) && (aml_recy != NULL) && (!aml_recy_flags_chk(AML_RECY_STATE_ONGOING))
        && (aml_recy->reconnect_rest == 0)
        && (aml_vif->vif_index == aml_recy->assoc_info.vif_idx)) {
...
#undef IS_BASIC_RATE
    ...
    if (ht_cap_ie) {
        struct ieee80211_ht_cap *ht_cap = (void *)(ht_cap_ie + 2);
...
#endif
    error = aml_beacon_update_pre(vif, &buf, bcn_buf, bcn->len);
    if (error) {
        kfree(bcn_buf);
        aml_msg_free(aml_hw, req);
        if (!(aml_recy_flags_chk(AML_RECY_STATE_ONGOING) &&
...
int aml_send_extra_ssid_param(struct aml_hw *aml_hw, struct cfg80211_scan_request *param)
{
    struct extra_ssid_param *ssid_param;
    int i;

    ssid_param = aml_priv_msg_zalloc(MM_SUB_SET_EXTRA_SSID_PARAM, sizeof(struct extra_ssid_param));
    if (!ssid_param)
        return -ENOMEM;
    ssid_param->ssid_cnt = (u8)min_t(int, EXTRA_SCAN_SSID_MAX, (param->n_ssids - SCAN_SSID_MAX));
    for (i = 0; i < ssid_param->ssid_cnt; i++) {
        int j;
        /* coverity[deref_overflow] - param->ssids is related to param->n_ssids */
        for (j = 0; j < param->ssids[i + SCAN_SSID_MAX].ssid_len; j++)
            ssid_param->ssid[i].array[j] = param->ssids[i + SCAN_SSID_MAX].ssid[j];
        ssid_param->ssid[i].length = param->ssids[i + SCAN_SSID_MAX].ssid_len;
        if (ssid_param->ssid[i].length) {
            AML_INFO("i:%d len:%d ssid:%s", i, ssid_param->ssid[i].length, ssid_param->ssid[i].array);
        }
    }

    /* coverity[leaked_storage] - req will be freed later */
    return aml_priv_send_msg(aml_hw, ssid_param, 0, 0, NULL);
}
                        ...
                        struct cfg80211_scan_request *param)
{
    ...
    AML_INFO("vif:%d ssid num %d\n", aml_vif->vif_index, param->n_ssids);
    ...
    if (param->n_ssids > SCAN_SSID_MAX) {
        int ret = 0;
        /*send extra ssid*/
        ret = aml_send_extra_ssid_param(aml_hw, param);
        if (ret) {
            AML_INFO("send extra ssid param fail");
        }
    }
    req->ssid_cnt = (u8)min_t(int, SCAN_SSID_MAX, param->n_ssids);
    req->bssid = mac_addr_bcst;
    ...
    for (i = 0; i < req->ssid_cnt; i++) {
        ...
        if (req->ssid[i].length) {
            AML_INFO("i:%d len:%d ssid:%s", i, req->ssid[i].length, req->ssid[i].array);
        }
    }
    ...
    for (i = 0; i < req->chan_cnt; i++) {
        ...
        if (regdom_en != 1) {
            ...
            /* coverity[OVERRUN] channel is not equal to 0 */
            aml_regdom_table_pwr_get(aml_ieee80211_freq_to_chan(req->chan[i].freq, req->chan[i].band), req->chan[i].band, &ofdm_power, &dsss_power);
            ofdm_power = ofdm_power & 0x7f;
            dsss_power = dsss_power & 0x7f;

            if (req->chan[i].band == NL80211_BAND_2GHZ) {
                req->chan[i].tx_power = aml_regdom_set_pwr(chan_to_fw_pwr(chan->max_reg_power), aml_regdom_set_pwr(dsss_power, ofdm_power));
            } else {
                req->chan[i].tx_power = aml_regdom_set_pwr(chan_to_fw_pwr(chan->max_reg_power), ofdm_power);
            }
        }
    }
...
unsigned int aml_efuse_read(struct aml_hw *aml_hw, u32 addr)
{
    ...
    req = aml_priv_msg_zalloc(MM_SUB_GET_EFUSE, sizeof(struct get_efuse_req));
    if (!req)
        return -ENOMEM;
    ...
    if (ind.addr != addr) {
         AML_ERR("read_efuse:%x error!\n", ind.addr);
    }
    AML_INFO("read_efuse:addr = 0x%x, value = 0x%x\n", addr, ind.value);
...
static struct aml_wake_reason_id aml_wake_reason_tbl[AML_WAKE_REASON_NUM] = {
    {0x00, "RXL_BCN"},
    {0x01, "UPLOAD_FRAME"},
    {0x02, "UPLOAD_MSG"},
    {0x03, "DEFAULT"},
...
static char *aml_wake_reason_2str(unsigned int wake_host_reason)
{
    int i;

    for (i = 0; i < AML_WAKE_REASON_NUM; i++) {
        if (aml_wake_reason_tbl[i].wake_reason_id == wake_host_reason)
            return aml_wake_reason_tbl[i].name;
    }
...
int aml_send_suspend_req(struct aml_hw *aml_hw, u8_l filter, enum wifi_suspend_state state)
{
    ...
    struct resume_to_host_cfm cfm;

    memset(&cfm, 0, sizeof(struct resume_to_host_cfm));
    ...
    if (state == WIFI_SUSPEND_STATE_NONE) {
        /* coverity[leaked_storage] - req will be freed later */
        ret = aml_priv_send_msg(aml_hw, req, 1, PRIV_SET_RESUME_CFM, &cfm);

        if (ret) {
            AML_INFO("resume send fail!\n");
        } else {
            AML_INFO("received resume_cfm, wake host reason: %s\n",
                    aml_wake_reason_2str(cfm.wake_host_reason));
        }
    } else {
        /* Send the MM_SUB_SET_SUSPEND_REQ message to FW */
        // MSG_BYPASS: insmod module, not open interface
        /* coverity[leaked_storage] - req will be freed later */
        ret = aml_priv_send_msg(aml_hw, req, 0, MM_MSG_BYPASS_ID, NULL);
    }
...
static int aml_get_sched_scan_req(struct aml_vif *aml_vif,
                                  ...
                                  struct scanu_sched_scan_start_req *sched_start_req)
{
    ...
    if (!request->match_sets || !request->scan_plans) {
        AML_INFO("match_sets %p, scan_plans %p", (void *)request->match_sets, (void *)request->scan_plans);
        return -EINVAL;
    }
...
int aml_send_sched_scan_stop_req(struct aml_vif *aml_vif, u64 reqid)
{
    ...
    /* coverity[leaked_storage] - sched_start_req will be freed later */
    return aml_priv_send_msg(aml_hw, sched_scan_stop_req, 1, PRIV_SCHED_SCAN_CANCEL_CFM, NULL);
}
...
{
    u32 *req = NULL;
    req = aml_priv_msg_zalloc(MM_SUB_SET_PROT_TYPE, sizeof(u32));
    if (!req)
      return -ENOMEM;
...
{
    void *void_param = NULL;

    void_param = aml_priv_msg_zalloc(MM_SUB_TRIG_SEC_TEST, sizeof(uint32_t));
    if (!void_param)
        return -ENOMEM;

    /* coverity[leaked_storage] - pval will be freed later */
...
{
    int ret, i;
    struct mm_mdns_set_wake_ports *req = aml_priv_msg_zalloc(MDNS_SET_WAKE_PORTS, sizeof(struct mm_mdns_set_wake_ports));
    ...
    req->num = ports->num;
    for (i = 0; i < req->num; i++) {
        req->list_wake_port[i].protocol = ports->port[i].protocol;
        req->list_wake_port[i].matcher = ports->port[i].matcher;
        req->list_wake_port[i].port = ports->port[i].portNumber;
    }
    ret = aml_priv_send_msg(aml_hw, req, 0, 0, NULL);
    /* coverity[leaked_storage] - req will be freed later */
    MDNS_OFFLOAD_DEBUG("%s ok exit %d\n", __func__, __LINE__);
...
int aml_coex_cmd(struct net_device *dev, u32_l coex_cmd, u32_l cmd_ctxt_1, u32_l cmd_ctxt_2)
{
    AML_WARN(
        "\n"
        "=======================================================\n"
        "!!! WARNING: 'coex_cmd' is NOT supported after version R3.4\n"
        "!!! Please use 'coex_mode' instead:\n"
        "!!!     0: AUTO\n"
        "!!!     1: TDD\n"
        "!!!     2: FDD\n"
        "======================================================="
...
int aml_set_coex_mode_cmd(struct net_device *dev, u32_l coex_cmd)
{
    ...
    AML_INFO("set coex mode cmd: %d", coex_cmd);
...
int aml_send_fwlog_cmd(struct aml_hw *aml_hw, int mode, struct fwlog_mode_cfm *cfm)
{
    ...
    AML_INFO("fwlog_param->mode:%d", fwlog_param->mode);
...
static void aml_sync_trace_cb(struct timer_list *t)
{
    struct aml_hw *aml_hw = from_timer(aml_hw, t, sync_trace_timer);

    aml_wq_do(aml_send_sync_trace, aml_hw);
...
int aml_set_linkloss_threshold(struct aml_hw * aml_hw, int threshold)
{
    ...
    if ((threshold < 3000 * 1024) && (threshold != 0)) {  // 3000 * TU ms, 0 is default
    ...
    AML_M_INFO(GENERIC, "set threshold:%d", threshold);

    /* coverity[leaked_storage] - mcc_ratio_param will be freed later */
    return aml_priv_send_msg(aml_hw, linkloss_threshold, 0, MM_MSG_BYPASS_ID, NULL);
}

int aml_set_2g4_bindwidth(struct aml_hw * aml_hw, u8 enable_2g4_20m)
{
    struct mm_2g4_bindwidth *req;

    if ((enable_2g4_20m != 0) && (enable_2g4_20m != 1)) {
        AML_M_WARN(GENERIC, "error param enable_2g4_20m:%d", enable_2g4_20m);
        return EINVAL;
    }

    req = aml_priv_msg_zalloc(MM_SUB_SET_2G4_BINDWIDTH, sizeof(struct mm_2g4_bindwidth));
    if (!req)
        return -ENOMEM;

    req->enable_2g4_20m = enable_2g4_20m;
...
{
    struct custom_conf_param *req;

    req = aml_priv_msg_zalloc(MM_SUB_SET_CUSTOM_CONF, sizeof(struct custom_conf_param));
    if (!req)
        return -ENOMEM;

    memcpy(req, &(aml_hw->custom_conf), sizeof(struct custom_conf_param));
    ...
    if (aml_bus_type == USB_MODE) {
            ...
            if (align_size != 0)
            {
                aml_plat->hif_sdio_ops->hi_random_ram_write((unsigned char *)program,
                    (unsigned char *)(uintptr_t)aml_hw->apf_params.apf_cap.apf_mem_addr, align_size);
            }

            if (program_len % 512 != 0)
            {
                aml_plat->hif_sdio_ops->hi_random_ram_write((unsigned char *)program + align_size,
                    (unsigned char *)(uintptr_t)(aml_hw->apf_params.apf_cap.apf_mem_addr + aml_hw->apf_params.apf_cap.max_len), program_len % 512);
            }
        } else {
...
int aml_set_early_suspend_mode(struct aml_hw *aml_hw, bool early_suspend_mode)
{
    ...
    /* coverity[leaked_storage] - early_suspend_mode_req will be freed later */
    return aml_priv_send_msg(aml_hw, early_suspend_mode_req, 0, 0, NULL);
}

int aml_regdom_en(struct aml_hw *aml_hw, uint32_t reg_en)
{
    struct regdom_en_req *req = NULL;

    req = aml_priv_msg_zalloc(MM_SUB_REGDOM_EN, sizeof(struct regdom_en_req));
    if (!req)
        return -ENOMEM;
    memset((void *)req, 0,sizeof(struct regdom_en_req));
    req->reg_en = reg_en;
...
{
    struct phy_maskfilter_cfg_param_t *req;
    unsigned char shift_power = 0;
    req = aml_priv_msg_zalloc(MM_SUB_PHY_CFG_MASKFILTER_REQ, sizeof(phy_maskfilter_cfg_param_t));

    if (!req)
        return -ENOMEM;

    shift_power = (ofdm_power >> 7);

    if (shift_power)
    {
        memcpy(req, &country_pwr_limit_cfg->phy_maskfilter_cfg[1], sizeof(phy_maskfilter_cfg_param_t));
    }
    else
    {
        memcpy(req, &country_pwr_limit_cfg->phy_maskfilter_cfg[0], sizeof(phy_maskfilter_cfg_param_t));
    }
...
int aml_get_radio_info(struct aml_hw *aml_hw, struct mm_get_radio_cfm *cfm)
{
    ...
    void_param = aml_priv_msg_zalloc(MM_SUB_GET_RADIO_INFO, 0);
    if (!void_param || !cfm)
        return -ENOMEM;

    memset(cfm, 0, sizeof(struct mm_get_radio_cfm));
